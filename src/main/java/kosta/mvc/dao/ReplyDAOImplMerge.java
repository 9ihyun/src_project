...
public class ReplyDAOImplMerge implements ReplyDAO {
	Properties proFile = new Properties();

	public ReplyDAOImplMerge() {
		try {
	...
	public int insertReply(PostReply reply) throws SQLException {
		...
		int result = 0;
		String sql = null;
		if (reply.getPostNo() > 20000) {
			sql = proFile.getProperty("reply.insertStudyReply");
		} else {
			sql = proFile.getProperty("reply.insertReply");
		}
	...
	public int updateReply(PostReply reply) throws SQLException {
		...
		String sql = null;
		if (reply.getPostNo() > 20000) {
			sql = proFile.getProperty("reply.updateStudyReply");
		} else {
			sql = proFile.getProperty("reply.updateReply");
		}
	...
	public int deleteReply(int replyNo) throws SQLException {
		...
		String sql = null;
		if (replyNo > 20000) {
			sql = proFile.getProperty("reply.deleteStudyReply");
		} else {
			sql = proFile.getProperty("reply.deleteReply");
		}
	...
	public List<PostReply> selectAllReply(int postNo) throws SQLException {
		...
		String sql = null;
		if (postNo > 20000) {
			sql = proFile.getProperty("reply.selectAllStudyReply");
		} else {
			sql = proFile.getProperty("reply.selectAllReply");
		}
	...
	public String getDBUserId(int replyNo) throws SQLException {
		...
		String sql = null;
		if (replyNo > 20000) {
			sql = proFile.getProperty("reply.getStudyDBUserId");
		} else {
			sql = proFile.getProperty("reply.getDBUserId");
		}
